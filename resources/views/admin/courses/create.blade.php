    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number_of_topics') ? 'is-invalid' : '' }}" type="number" name="number_of_topics" id="number_of_topics" value="{{ old('number_of_topics', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags" id="tags" required>
                    ...
                    @foreach(App\Models\Course::TAGS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tags', 'Beginner') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
                    <div class="invalid-feedback">

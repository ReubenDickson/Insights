...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.poolEarning.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pool-earnings.update", [$poolEarning->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('period') ? 'is-invalid' : '' }}" name="period_id" id="period_id" required>
                    @foreach($periods as $id => $entry)
                        <option value="{{ $id }}" {{ (old('period_id') ? old('period_id') : $poolEarning->period->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $poolEarning->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('direct_referal_earnings') ? 'is-invalid' : '' }}" type="number" name="direct_referal_earnings" id="direct_referal_earnings" value="{{ old('direct_referal_earnings', $poolEarning->direct_referal_earnings) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('first_tier_earnings') ? 'is-invalid' : '' }}" type="number" name="first_tier_earnings" id="first_tier_earnings" value="{{ old('first_tier_earnings', $poolEarning->first_tier_earnings) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('second_tier_earnings') ? 'is-invalid' : '' }}" type="number" name="second_tier_earnings" id="second_tier_earnings" value="{{ old('second_tier_earnings', $poolEarning->second_tier_earnings) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('third_tier_earnings') ? 'is-invalid' : '' }}" type="number" name="third_tier_earnings" id="third_tier_earnings" value="{{ old('third_tier_earnings', $poolEarning->third_tier_earnings) }}" step="0.01" required>

...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.secondTierAfilliate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.second-tier-afilliates.update", [$secondTierAfilliate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('email') ? 'is-invalid' : '' }}" name="email_id" id="email_id" required>
                    @foreach($emails as $id => $entry)
                        <option value="{{ $id }}" {{ (old('email_id') ? old('email_id') : $secondTierAfilliate->email->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_became_second_tier') ? 'is-invalid' : '' }}" type="text" name="date_became_second_tier" id="date_became_second_tier" value="{{ old('date_became_second_tier', $secondTierAfilliate->date_became_second_tier) }}" required>

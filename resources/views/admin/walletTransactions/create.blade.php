...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.walletTransaction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.wallet-transactions.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('transaction') ? 'is-invalid' : '' }}" name="transaction" id="transaction">{!! old('transaction') !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('balance') ? 'is-invalid' : '' }}" name="balance_id" id="balance_id" required>
                    @foreach($balances as $id => $entry)
                        <option value="{{ $id }}" {{ old('balance_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>

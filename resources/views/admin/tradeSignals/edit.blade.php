...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tradeSignal.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trade-signals.update", [$tradeSignal->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="crypto_pair">{{ trans('cruds.tradeSignal.fields.crypto_pair') }}</label>
                <input class="form-control {{ $errors->has('crypto_pair') ? 'is-invalid' : '' }}" type="text" name="crypto_pair" id="crypto_pair" value="{{ old('crypto_pair', $tradeSignal->crypto_pair) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('entry') ? 'is-invalid' : '' }}" type="text" name="entry" id="entry" value="{{ old('entry', $tradeSignal->entry) }}" required>
                @if($errors->has('entry'))
                    <div class="invalid-feedback">
                        {{ $errors->first('entry') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('exit') ? 'is-invalid' : '' }}" type="text" name="exit" id="exit" value="{{ old('exit', $tradeSignal->exit) }}" required>
                @if($errors->has('exit'))
                    <div class="invalid-feedback">
                        {{ $errors->first('exit') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('stop_loss') ? 'is-invalid' : '' }}" type="text" name="stop_loss" id="stop_loss" value="{{ old('stop_loss', $tradeSignal->stop_loss) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('comment') ? 'is-invalid' : '' }}" name="comment" id="comment">{!! old('comment', $tradeSignal->comment) !!}</textarea>

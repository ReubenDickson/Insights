...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.courseRating.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.course-ratings.update", [$courseRating->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('course') ? 'is-invalid' : '' }}" name="course_id" id="course_id" required>
                    @foreach($courses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('course_id') ? old('course_id') : $courseRating->course->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('average_ratings') ? 'is-invalid' : '' }}" type="number" name="average_ratings" id="average_ratings" value="{{ old('average_ratings', $courseRating->average_ratings) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_reviews') ? 'is-invalid' : '' }}" type="number" name="total_reviews" id="total_reviews" value="{{ old('total_reviews', $courseRating->total_reviews) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_stars') ? 'is-invalid' : '' }}" type="number" name="total_stars" id="total_stars" value="{{ old('total_stars', $courseRating->total_stars) }}" step="1">
